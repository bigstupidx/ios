...
- (void) addImagesFromResources:(NSArray *) names
{
    ...
    if (names.count == 0) {
        [self addImage:[UIImage imageNamed:@"PlaceHolderModa.png"]];
    }
    UIImageView* imgView = [[UIImageView alloc] init];
    for(NSString * name in names){
        
        NSURL* imgUrl = [NSURL URLWithString:[name stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        [imgView setImageWithURL:imgUrl placeholderImage:[UIImage imageNamed:@"PlaceHolderModa.png"] completed:nil];
    ...
    if(! _isAnimating &&
       [self.images count] >1){
        ...
        if (self.isStart && ([[NSUserDefaults standardUserDefaults] integerForKey:@"currentIndex"] != _currentIndex)) {
            [[GAI sharedInstance].defaultTracker send:[[GAIDictionaryBuilder createEventWithCategory:@"AdvertisementView" action:@"Show Top Ads View In Article Detail" label:self.imgURLArray[_currentIndex] value:nil] build]];
            self.isStart = NO;
