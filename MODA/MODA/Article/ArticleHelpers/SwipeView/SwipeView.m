...
- (void)didScroll
{
    ...
    {
        UIInterfaceOrientation newOri = [[UIApplication sharedApplication] statusBarOrientation];
        if (_curOrientation == newOri) {
            //update item index
            _currentItemIndex = [self clampedIndex:roundf(_scrollOffset)];
            
        }
        else {
            _curOrientation = newOri;
        }
        //NSLog(@"_currentItemIndex = %d", _currentItemIndex);
        //load views
        _lastUpdateOffset = _currentItemIndex;
        [self loadUnloadViews];
        
        //send index update event
        if (_previousItemIndex != _currentItemIndex || _currentItemIndex == 0)
        {
