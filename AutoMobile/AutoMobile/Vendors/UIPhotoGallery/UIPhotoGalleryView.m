...
- (void)startslideshow:(NSInteger)curindex isRepeat:(BOOL)repeat
{
    [self scrollToPage:curindex animated:YES];
    int64_t delayInSeconds = 3.0;
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, delayInSeconds * NSEC_PER_SEC);
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        if (repeat) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"playAgain" object:@(curindex)];
        }
            });
